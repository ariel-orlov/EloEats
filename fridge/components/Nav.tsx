...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-surface/95 backdrop-blur-md border-t border-divider flex lg:hidden z-50">
      {LINKS.map(({ href, label, icon }) => {
        ...
        return (
          <Link
            key={href}
            href={href}
            className="flex-1 flex flex-col items-center pt-2.5 pb-4 gap-1.5 relative"
          >
...
export function Sidebar() {
  const path = usePathname();
  return (
    <aside className="hidden lg:flex flex-col items-center w-16 bg-surface border-r border-divider min-h-screen py-5 gap-1 fixed left-0 top-0 z-50">
      {/* Logo mark */}
      <div className="w-9 h-9 rounded-[10px] bg-primary flex items-center justify-center mb-5">
        <span className="text-white font-extrabold text-sm tracking-tight">FW</span>
      </div>
      {LINKS.map(({ href, label, icon }) => {
        const active = path.startsWith(href);
        return (
